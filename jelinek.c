            ...
            unsigned int maxcycles)/* Decoding timeout in cycles per bit */
{
    ...
    for (i=1; i <= ncycles; i++) {
        ...
        bucket=(totmet0>>6)+200; //may not work on all compilers - totmet can be negative
        ...
        // extending the size of the stack by 1
        if( depth <= nbits_minus_ntail ) {
            ...
            stack[ptr].depth=depth;
            bucket=(totmet1>>6)+200; //this may not be safe on all compilers
            if( bucket > high_bucket ) high_bucket=bucket;
            if( bucket < low_bucket ) low_bucket=bucket;
        ...
        // at this point the pure stack algorithm would sort the stack and pick
        // off the top entry. The stack-bucket approximation picks off the latest
        // entry from the high bucket.
